import { getOptions, debounce } from './utils/jsTools.js';
...
const onSearch = async ({ target: { value } }) => {
  ...
  const cards = await Promise.all(projects.map(async project => await followCard('ch-utils-quickSearch-result', project)));
  document.getElementById('ch-utils-quickSearch-projects').replaceChildren(...cards);
};

const searchWindow = noact({
  tag: 'dialog',
  className: 'ch-utils-quickSearch h-full w-full max-h-screen max-w-screen flex-col items-start justify-stretch gap-6 bg-transparent px-0 py-28 fixed top-0 text-inherit open:flex lg:px-6',
  style: 'background: rgb(25 25 25 / .9)',
  onclick: event => {
    try {
      event.stopPropagation();
      if (event.target.matches('dialog')) searchWindow.removeAttribute('open');
    } catch {};
  },
  children: [{
    ...
    children: [{
      ...
      children: [
        ...
        {
          className: 'ch-utils-quickSearch-placeHolder h5',
          children: ['enter a query to see results!']
        },
        {
          id: 'ch-utils-quickSearch-results',
          children: [
            {
              className: 'mt-4',
              children: [
                ...
                {
                  id: 'ch-utils-quickSearch-projects',
